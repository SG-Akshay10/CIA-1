...
def recursive_compare(word1,word2,i,j,matrix):
    ...
    if word1[i - 1] == word2[j - 1]:
        matrix[i][j] = recursive_compare(word1, word2, i - 1, j - 1, matrix) + 5
    else:
        matrix[i][j] = max(recursive_compare(word1, word2, i - 1, j, matrix), recursive_compare(word1, word2, i, j - 1, matrix), recursive_compare(word1, word2, i - 1, j - 1, matrix)) - 4
    return matrix[i][j]

def compare_strings(string1, string2):
    matrix = np.full((len(string1) + 1, len(string2) + 1), 0)
